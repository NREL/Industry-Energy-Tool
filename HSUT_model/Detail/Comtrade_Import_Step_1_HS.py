...
################################ Import data #################################
#import data
Comtrade_HS_data = "Imports_2012_Comtrade_HS_6dig.xlsx"
...
# data imports
Imports_HS_2012 = pd.read_excel(Comtrade_HS_data, header = 0, usecols = "B, J, M, V:Z, AD, AF")
...
Imports_HS_2012["Commodity Code"] = Imports_HS_2012["Commodity Code"].astype(str)
Imports_HS_2012["Commodity Code"] = Imports_HS_2012["Commodity Code"].apply(fill_zeros)

# Only want values with values in kg
Imports_HS_2012 = Imports_HS_2012[Imports_HS_2012['Qty Unit Code'] == 8]
...
Imports_2012_NAICS_from_HS = pd.merge(Imports_HS_2012, HS_NAICS_Conc_Usable, left_on = ['Commodity Code'], right_index = True, how = 'left')


######## 

Imports_2012_HS_EZ = Imports_2012_NAICS_from_HS[Imports_2012_NAICS_from_HS['NAICS'].notna()]
Imports_2012_HS_EZ = Make_NAICS_Cols(Imports_2012_HS_EZ) 

pdListHS = Sim_conditional_merge(Imports_2012_HS_EZ, BEA2012)
Imports_BEA_dupes_HS = pd.merge(Imports_2012_NAICS_from_HS, HS_Dupe_Conc, left_on = ['Commodity Code'], right_on = ['HS'])

Imports_BEA_HS = pd.concat(pdListHS)
Imports_BEA_HS = pd.concat([Imports_BEA_HS, Imports_BEA_dupes_HS])

### Now that the imports have been assigned to BEA codes, compute price vectors for each BEA code
Imports_BEA_HS.Detail = Imports_BEA_HS.Detail.map(str)

HS_Price_Vector = Imports_BEA_HS.groupby(by = ["Detail"]).sum()
HS_Price_Vector['Price'] = HS_Price_Vector['Trade Value (US$)']/HS_Price_Vector['Netweight (kg)']
HS_Price_Vector.index = HS_Price_Vector.index.map(str)


# # Store price vector in excel
writer = "Price_Vector_HS.xlsx"
