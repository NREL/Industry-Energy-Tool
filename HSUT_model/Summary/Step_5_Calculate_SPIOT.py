...
Make_Phys = 'Physical_Make_Summary.xlsx'
q_Phys = 'Physical_q.xlsx'
e_Phys = 'Physical_e.xlsx'
x_Phys = 'Physical_x.xlsx'
A_Mon = 'A_Monetary_SIOT.xlsx'
Inverse_Price_Matrix = 'Inverse_Price_Matrix_Use.xlsx'

U_dict = pd.read_excel(Use_Phys, sheet_name = None, header = 0, index_col = 0)
V_dict = pd.read_excel(Make_Phys, sheet_name = None, header = 0, index_col = 0)
q_dict = pd.read_excel(q_Phys, sheet_name = None, header = 0, index_col = 0)
e_dict = pd.read_excel(e_Phys, sheet_name = None, header = 0, index_col = 0)
g_dict = pd.read_excel(x_Phys, sheet_name = None, header = 0, index_col = 0)
A_Mon_dict = pd.read_excel(A_Mon, sheet_name = None, header = 0, index_col = 0)
Inverse_Price_Matrix = pd.read_excel(Inverse_Price_Matrix, sheet_name = None, header = 0, index_col = 0)
...
def drop_entries_dict(dict_df, list_entries):
    newdict = {}
    for key, df in dict_df.items():
...
A_phys_dict = mult_df_dicts(Arg1, Arg2)

A_Mon_New = drop_entries_dict(A_Mon_dict, List_years)
A_Mon_transpose = transpose_dict(A_Mon_New)

Arg3 = mult_df_dicts(A_Mon_transpose, Inverse_Price_Matrix)
Z_phys = transpose_dict(Arg3)

dict_to_xls(Z_phys, 'Z_Physical_SIOT.xlsx')
dict_to_xls(A_phys_dict, 'A_Physical_SIOT.xlsx')
